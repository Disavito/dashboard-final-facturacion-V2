...
/**
 * Helper para calcular el valor base (sin IGV) a partir del precio total (con IGV).
 */
const calculateBaseValue = (totalPrice: number, igvPercentage: number): number => {
  ...
  // Base Value = Total Price / (1 + IGV Rate)
...
const DEFAULT_IGV_PERCENTAGE = 18; // Porcentaje estándar para afectación '10'

// Nuevo tipo para rastrear la boleta emitida
...
const defaultValues: BoletaFormValues = {
  ...
  usuario_creacion: 'admin_user', // Placeholder, ideally fetched from context
...
const BoletaForm = () => {
  ...
  // Handler para la generación del PDF
  const handleGeneratePdf = useCallback(async (boletaId: number) => {
    ...
    try {
      ...
      toast({
        ...
        description: "El PDF se está generando en el servidor. Puede tardar unos segundos.",
        variant: "success",
      });
    } catch (error) {
  ...
  // Handler para la descarga del PDF
  const handleDownloadPdf = async () => {
    if (!lastIssuedBoleta) return;
    
    try {
      // La función de API maneja la interacción con el navegador (blob, link click)
      await downloadBoletaPdf(lastIssuedBoleta.id, lastIssuedBoleta.numero_completo, 'A4');
  ...
  const onSubmit = async (data: BoletaFormValues) => {
    ...
    setLastIssuedBoleta(null); // Limpiar resultado anterior
    
    // CRÍTICO: Procesar detalles para convertir el Precio Total (con IGV) 
    ...
    try {
      ...
      const boletaId = result.data.id; // ID interno
      const numeroCompleto = result.data.numero_completo;
      
      // 1. Almacenar el resultado para acciones de PDF
      setLastIssuedBoleta({ id: boletaId, numero_completo: numeroCompleto });

      // 2. Iniciar la generación del PDF
  ...
  // Función para buscar cliente (sin cambios)
  const handleClientSearch = useCallback(async (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    // Solo buscar si es DNI (tipo '1') y tiene la longitud mínima (8)
  ...
  }, [form, toast, fetchClientByDocument]);
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 bg-card border-border rounded-xl shadow-lg" align="start">
                      <Calendar
                        ...
                        initialFocus
                        locale={es}
                        toDate={new Date()} // No permitir fechas futuras
                      />
                    </PopoverContent>
                  </Popover>
                  <FormMessage />
        ...
        {lastIssuedBoleta && (
          <Card className="bg-surface border-success/30 shadow-xl">
            ...
            <CardContent className="flex flex-col md:flex-row gap-4">
              ...
              <Button 
                type="button" 
                onClick={() => { form.reset(defaultValues); setLastIssuedBoleta(null); }}
                variant="outline"
